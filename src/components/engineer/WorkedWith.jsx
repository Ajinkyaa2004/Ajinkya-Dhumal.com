  ...
  <section aria-label="Worked with" className="relative px-6 md:px-20 z-10 max-w-6xl mx-auto pb-10 md:pb-16">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="border-t border-b border-white/[0.06] py-8">
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-5 md:gap-x-16">
        {WORKED_WITH.map((c) =>
          c.logo ? (
            <img key={c.name} src={c.logo} alt={c.name} loading="lazy" className="h-7 md:h-9 w-auto object-contain opacity-50 hover:opacity-95 grayscale hover:grayscale-0 transition-all duration-300" />
          ) : (
            <span key={c.name} className="text-lg md:text-2xl font-extrabold tracking-tight text-white/35 hover:text-white/80 transition-colors duration-300 cursor-default whitespace-nowrap">
              {c.name}
            </span>
          )
        )}
